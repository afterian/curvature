...
""" fucking windows not saving my shelf buttons. 
import cva
reload(cva)
cva.cvaUI()
...
#defining the scary global vars. until i find a way to deal with them. 
curvesel = ""
curveselShape =""
cvaName = ""
poca = ""
...
def cvaUI():    
    ...
    #create a slave      
    masterName= cmds.textField("slaveName", tx = "slave_aaa")
    cmds.button(command = createSlave, label = "Create Slave", w = 300, h = 20)
...
def addAssetTab (assetID,isRef):
    ...
    getSlaveAss="%s:slave_aaa"%myNameSpace
    cmds.select(mSel[0],getSlaveAss)
    attachMS()    
...
def createMS(masterName,typeMS): # this is the def for createing Master Slave items. 
...
def createSlave(masterName):
    masterName = cmds.textField("slaveName", q=True, text = True)#get name from text feild
    typeMS="slave"#setting the creation type.
    slaveOB = createMS(masterName,typeMS)
...
def attachMS (*args):
    ...
    #cmds.select(slave)
    #slaveGrp= cmds.pickWalk(d="up")#sets the incoming connection to its _in node. 
    
    slaveGrp = slave + "_m"
    print slaveGrp
    
    #constraints
    cmds.select(master,slaveGrp)
    cmds.parentConstraint (master,slaveGrp, mo=False)
    cmds.scaleConstraint(master,slaveGrp, mo = False)
    
    #connect drive. 
    if doConnectDrive == True:
        cmds.connectAttr(master +".drive", slaveGrp + ".drive")
    #cmds.connectAttr(master +".uniform_scale", slave + ".uniform_scale")
def attachMSOFF (*args):
    ...
    #constraints
    cmds.select(master,slaveGrp)
    cmds.parentConstraint (master,slaveGrp, mo=True)
    cmds.scaleConstraint(master,slaveGrp, mo = True)
...
def addAssetCVA(iX,iY,inMode):
    ...
    if refAss == True:
        ...
        if connectCVA == True:
            getSlaveAss="%s:slave_aaa"%nameSpaceRef
            cmds.select(lastMScreated,getSlaveAss)
            attachMS()
...
def addMultiAssetCVA(assetID,isRef):
    ...
    for iray in xrange (0,aCount):
         getSlaveAss="%s:loc_%s_aaa"% (myNameSpace,iray)
         cmds.select(mSel[iray],getSlaveAss)
         attachMS()
